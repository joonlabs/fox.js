...
export class CollisionManager {
    ...
    static colliding({obj1, obj2} = {}) {
        ...
        if (obj1.constructor.name === "RectangleCollider" && obj2.constructor.name === "RectangleCollider") {
             ...
             */
            if (calcAxis({axis: axis1, A: A, B: B}) && calcAxis({axis: axis2, A: A, B: B}) && calcAxis({
                axis: axis3,
                A: A,
                B: B
            }) && calcAxis({axis: axis4, A: A, B: B})) {
                let intersections = []
            ...
            function getIntersectionsWithLine({line, circle} = {}) {
                ...
                if (((circle.dimensions.width / 2) ** 2 * dr ** 2 - D ** 2) > 0) {
                    ...
                    let cord1 = {
                        x: x_1 + circle.position.x + (circle.dimensions.width / 2),
                        y: y_1 + circle.position.y + (circle.dimensions.width / 2)
                    }
                    let cord2 = {
                        x: x_2 + circle.position.x + (circle.dimensions.width / 2),
                        y: y_2 + circle.position.y + (circle.dimensions.width / 2)
                    }

                    if (isABetweenA1A2({a: x_1, a1: x1, a2: x2}) && isABetweenA1A2({
                        a: y_1,
                        a1: y1,
                        a2: y2
                    })) intersections.push(cord1)
                    if (isABetweenA1A2({a: x_2, a1: x1, a2: x2}) && isABetweenA1A2({
                        a: y_2,
                        a1: y1,
                        a2: y2
                    })) intersections.push(cord2)
                    return intersections
                }
            ...
            if (distance <= circle_.radius ** 2) {
                //intersection found
                let line1 = {
                    x1: rect.getCorners().upperLeft.x,
                    y1: rect.getCorners().upperLeft.y,
                    x2: rect.getCorners().upperRight.x,
                    y2: rect.getCorners().upperRight.y
                }
                let line2 = {
                    x1: rect.getCorners().upperRight.x,
                    y1: rect.getCorners().upperRight.y,
                    x2: rect.getCorners().lowerRight.x,
                    y2: rect.getCorners().lowerRight.y
                }
                let line3 = {
                    x1: rect.getCorners().lowerRight.x,
                    y1: rect.getCorners().lowerRight.y,
                    x2: rect.getCorners().lowerLeft.x,
                    y2: rect.getCorners().lowerLeft.y
                }
                let line4 = {
                    x1: rect.getCorners().lowerLeft.x,
                    y1: rect.getCorners().lowerLeft.y,
                    x2: rect.getCorners().upperLeft.x,
                    y2: rect.getCorners().upperLeft.y
                }
