...
export class ParticleSystem extends GameObject{
     ...
     * @param {function} distribution Function that is called for every particle, when it is initiated, or resetted 
     ...
     * @returns Particle
     */
    constructor({x, y, width, height, rotation, rotationPosition, distribution, initiation, settings, layer, z, tag}={settings:{maxParticles: 1000, particleLifespan: 60, spawnRate: 10}}){
        super({
            x:(x==undefined?0:x), 
            y:(y==undefined?0:y), 
        ...
        for(let i=0; i<this.settings.maxParticles; i++){ 
            let particle = new Particle({layer:layer})
    ...
    render({x, y, width, height, zoom, camera, renderer}, _this=this){
        if(_this.state!=0){
            for(let particle of _this.pool){
                if(!particle.dead) particle.render({x:x, y:y, width: parseInt(particle.dimensions.width*zoom), height: parseInt(particle.dimensions.height*zoom), zoom:zoom, camera:camera, renderer:renderer})
            }
        }
    }
}