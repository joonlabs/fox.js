...
export class Sprite extends GameObject{
    ...
    constructor({x, y, width, height, rotation, rotationPosition, layer, tag, texture, z, debug}={}){
        super({x:x, y:y, width:width, height:height, rotation:rotation, rotationPosition:rotationPosition, layer:layer, tag:tag, z:z, debug:debug})
    ...
    render({x, y, width, height, zoom, camera, renderer}={}, _this=this){
        ...
        let rotationPosition_ = _this.rotationPosition.multS({scalar: zoom})
        
        if(_this.texture.loaded){
            renderer.renderTexture({texture: _this.rendering.canvas, x:x, y:y, width: width, height: height, rotation: _this.rotation, rotationPosition: rotationPosition_, ctx:_this.layer.ctx})
        }
        if(_this.debug.enabled && _this.debug.hitbox){
            renderer.strokeRect({x:x, y:y, width:width, height:height, color:"#de5a1f", rotation:_this.rotation, rotationPosition:rotationPosition_, lineWidth: parseInt(4*zoom), ctx:_this.layer.ctx})
        }
